...
class Hashmap {
    ...
    get(index){
        let output=[]
        let current=myhashmap.map[index].head;
        while(current){
    ...
    containes(index){
        if(myhashmap.map[index]==undefined){
    ...
    keys(){
        let output=[];
        myhashmap.map.forEach((current)=>{
            output.push(Object.keys(current.head.value)[0])
            if(current.head.next!==null){
                output.push(Object.keys(current.head.next.value)[0])
            }
        })
        return console.log(output)
    }
